...
class RoboschoolForwardWalker(SharedMemoryClientEnv, BaseModelBasedEnv):
    ...
    def step(self, a):
        reward = -self.cost_np_vec(self._old_state[None, :], a[None, :], self._old_state[None, :] * 0.0)[0]
        ...
        self.frame  += 1
        if (done and not self.done) or self.frame==1000:
            self.episode_over(self.frame)
        self.done   += done   # 2 == 1+True
        self.reward += sum(self.rewards)
        self.HUD(state, a, done)
        self._old_state = np.concatenate([state, [progress]])
        return np.concatenate([state, [progress]]), reward, bool(done), {}

    def episode_over(self, frames):
        pass

    def cost_np_vec(self, obs, acts, next_obs):
        reward_ctrl = -0.003 * np.sum(np.square(acts), axis=1)
        reward_run = obs[:, -1]
        reward = reward_run + reward_ctrl + self._add_alive_bonus
        return -reward
...
class RoboschoolForwardWalkerMujocoXML(RoboschoolForwardWalker, RoboschoolMujocoXmlEnv):
    ...
    def reset(self):
        s = RoboschoolMujocoXmlEnv.reset(self)
        self._old_state = np.concatenate([s, [0.0]])
...
class RoboschoolHalfCheetah(RoboschoolForwardWalkerMujocoXML):
    ...
    def __init__(self):
        self._add_alive_bonus = 0.0
        RoboschoolForwardWalkerMujocoXML.__init__(self, "half_cheetah.xml", "torso", action_dim=6, obs_dim=27, power=0.90)
...
class RoboschoolAnt(RoboschoolForwardWalkerMujocoXML):
    ...
    def __init__(self):
        self._add_alive_bonus = 1.0
        RoboschoolForwardWalkerMujocoXML.__init__(self, "ant.xml", "torso", action_dim=8, obs_dim=29, power=2.5)
    def alive_bonus(self, z, pitch):
